...
public class TokenFactory {
    ...
    public String createToken(Integer expirationMinutes, Claims claims, Boolean encryptPayload){
        ...
        if (encryptPayload){
            return Jwts.builder().issuer("Keplara").expiration(date).issuedAt(new Date()).encryptWith(Keys.password(payloadKey.toCharArray()), alg, enc).claims(claims).compact();
        } else {
            return Jwts.builder().issuer("Keplara").expiration(date).issuedAt(new Date()).signWith(getSigningKey()).claims(claims).compact();
        }
    }
